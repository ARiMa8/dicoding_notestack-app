...
async function fetchWithToken(url, options = {}) {
  ...
  console.log("fetchWithToken - URL:", url, "Token:", token);
...
async function getUserLogged() {
  try {
    ...
    if (status !== "success") {
      throw new Error(message || "Gagal mendapatkan data pengguna");
    }

    const user = data ? data.user || data : null;

    if (!user) {
      throw new Error("Data pengguna tidak ditemukan dalam respons");
    }
